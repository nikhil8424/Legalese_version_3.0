...
# Set your Gemini API key here (use environment variables in production)
genai.configure(api_key="AIzaSyA35EwyuTs2aasHoDnLpon7I6k_f5S1dpU") # Replace with your actual Gemini API key
...
# Global variable to store the translated text
...
# Simple matching based chatbot logic
def chatbot_response(user_input):
    global global_translated_text
    user_input = user_input.lower()
    
    if not global_translated_text:
        return "I don't have any document context yet. Please upload a document first."

    best_match = ""
    for sentence in global_translated_text.split("."):
        cleaned_sentence = clean_text(sentence).lower()
        if user_input in cleaned_sentence:
           best_match = sentence
           break
    ...
    if best_match:
        ...
        return "I'm sorry, I didn't find any relevant information."


# Gemini chatbot function
def gemini_chatbot(query, text):
    try:
         prompt = f"""
        You are a helpful chatbot designed to answer questions based on a given article.
        Here is the article:
        {text}

        Now, answer the following question based on the article above:
        {query}
        """
...
def upload():
    ...
    if file:
        try:
            ...
            global_translated_text = translated_text  # Store translated text in global variable
...
def gemini_chat():
    ...
    if not extracted_text:
        return jsonify({'response': 'No document has been uploaded yet'})
    if not user_input:
        return jsonify({'error':'No message was provided'})

    try:
       
        response = gemini_chatbot(user_input, extracted_text)
        return jsonify({'response': response})
    except Exception as e:
         return jsonify({'error': f"An error occurred in gemini: {e}"})
